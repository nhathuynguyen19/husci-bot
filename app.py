...
husc_notification = HUSCNotifications(login_url, 
                                      data_url[0], 
                                      bot_config.fixed_key, 
                                      notifications_path)
commands = Commands(husc_notification, 
                             user_manager, 
                             auth_manager)
reminders = Reminder(reminders_path, 
                             sent_reminders_path, 
                             bot)
loops = Loops(husc_notification, 
                          user_manager, 
                          auth_manager, 
                          bot)
events = Events(bot)
...
@bot.tree.command(name="login",
                  description="Đăng nhập HUSC")
async def login(ctx, 
                username: str, 
                password: str):
    await commands.handle_login(ctx, 
                                username, 
                                password)

@bot.tree.command(name="notifications", 
                  description="Lấy các thông báo mới từ HUSC")
async def notifications(ctx: discord.Interaction):
    await commands.handle_notifications(ctx)

@bot.tree.command(name="first", 
                  description="Lấy thông báo mới nhất từ HUSC")
...
async def remind(ctx: discord.Interaction, 
                 reminder: str, 
                 day: int, 
                 month: int, 
                 year: int, 
                 hour: int, 
                 minute: int):   
    date_time = datetime(int(year), 
                         int(month), 
                         int(day), 
                         int(hour), 
                         int(minute))
    await commands.handle_remind(ctx, 
                                 bot, 
                                 reminder, 
                                 reminders, 
                                 date_time)
...
@bot.event
async def on_ready():
    events.start(auto_notifications, 
                 reminder_loop, 
                 update_guilds_info)
